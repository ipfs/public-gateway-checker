...
`,ia=42;function na(e){if(e[0]!==0)throw new Error("Invalid CID for CBOR tag 42; expected leading 0x00");return Ne.decode(e.subarray(1))}var Vn={allowIndefinite:!1,coerceUndefinedToNull:!0,allowNaN:!1,allowInfinity:!1,allowBigInt:!0,strict:!0,useMaps:!1,tags:[]};Vn.tags[ia]=na;var ra=e=>ms(e,Vn),oa=Dn($r(),1),sa="Earth";function aa(e){return{country_name:e[0],country_code:e[1],region_code:e[2],city:e[3],postal_code:e[4],latitude:e[5],longitude:e[6],planet:sa}}var fa=Ne.parse("bafyreihnpl7ami7esahkfdnemm6idx4r2n6u3apmtcrxlqwuapgjsciihy"),ca=["https://ipfs.io","https://dweb.link"];async function la(e,t){if(typeof e=="function")return e(t);if(typeof e?.block?.get=="function")return e.block.get(t);let r=Array.isArray(e)?e:[e];for(let i of r){let c=new URL(i);c.pathname=`/ipfs/${t.toString()}`,c.search="?format=raw";try{let p=await(0,oa.default)(c,{headers:{Accept:"application/vnd.ipld.raw"},cache:"force-cache"});if(!p.ok)throw p;return new Uint8Array(await p.arrayBuffer())}catch(p){throw new Error(`unable to fetch raw block for CID ${t}`,{cause:p})}}}async function Hn(e,t,r=1){try{let i=await la(e,t);return{obj:await ra(i),block:i}}catch(i){if(r<3)return await Hn(e,t,r+1);throw i}}async function ua(e,t,r){let i,c;try{({obj:i,block:c}=await Hn(e,t))}catch(I){throw(process?.env?.DEBUG||process?.env?.TEST)&&console.error(c?`[ipfs-geoip] failed to parse DAG-CBOR behind CID '${t}'`:`[ipfs-geoip] failed to get raw block for CID '${t}'`,I),I}let p=0;if("data"in i){if("data"in i){for(;i.data[p]&&i.data[p].min<=r;)p++;let I=i.data[p-1];if(!I)throw new Error("Failed to lookup leaf node");if(!I.data)throw new Error("Unmapped range");return aa(I.data)}}else{for(;i.mins[p]&&i.mins[p]<=r;)p++;let I=i.links[p-1];if(!I)throw new Error("Failed to lookup node");let f=ha(I);if(!f)throw new Error("Failed to lookup node");return Wn(e,f,r)}}var Wn=Qr(ua,{cachePromiseRejection:!1,cacheKey:e=>{let[,t,r]=e;return`${t}.${r}`}});function qn(e=ca,t){return Wn(e,fa,Zr.default.toLong(t))}function ha(e){return e?Ne.asCID(e):null}var It=class{constructor({tokensPerInterval:t,interval:r,stopped:i}){if(this.tokensPerInterval=t,this._isStopped=i,typeof r=="string")switch(r){case"sec":case"second":{this.interval=1e3;break}case"min":case"minute":{this.interval=1e3*60;break}case"hr":case"hour":{this.interval=1e3*60*60;break}case"day":{this.interval=1e3*60*60*24;break}default:throw new Error("Invalid interval "+r)}else this.interval=r;this.tokensRemovedAt=[],this.awaitingRestart=new Set}dripTokens(){throw new Error("dripTokens() not implemented")}getDelayForTokens(){throw new Error("getDelayForTokens() not implemented")}getTokens(){return this.dripTokens(),Math.floor(this.tokens)}tryRemoveTokens(t){return this.dripTokens(),t>this.tokens?!1:(this.tokens-=t,this.tokensRemovedAt.push([Date.now(),t]),!0)}async awaitTokens(t=1){this._isStopped&&await new Promise(i=>this.awaitingRestart.add(i));let r=this.getDelayForTokens(t);return r===0?Math.floor(this.tokens):(await new Promise(i=>setTimeout(()=>i(),r)),this.awaitTokens(t))}reset(){this.awaitingRestart.forEach(t=>t())}stop(t=!0){this._isStopped=!0,t&&this.tryRemoveTokens(this.getTokens())}start(){this._isStopped=!1}get isStopped(){return this._isStopped}};var Ut=class extends It{constructor({bucketSize:t,tokensPerInterval:r,interval:i,stopped:c=!1}){super({tokensPerInterval:r,interval:i,stopped:c}),this.bucketSize=t,this.tokens=c?0:this.bucketSize,this.tokensRemovedAt=c?[0]:[Date.now()]}dripTokens(){if(this._isStopped)return;let t=Date.now(),r=this.tokensRemovedAt[0],i=Math.max(t-r,0);this.tokensRemovedAt=[t];let c=i*(this.tokensPerInterval/this.interval);return this.tokens=Math.min(this.tokens+c,this.bucketSize),Math.floor(this.tokens)}getDelayForTokens(t=1){if(t>this.bucketSize)throw new Error(`Cannot supply ${t} tokens at once (max is bucketSize = ${this.bucketSize}`);this.dripTokens();let r=t-this.tokens;if(r<=0)return 0;if(!this._isStopped)return Math.ceil(r*(this.interval/this.tokensPerInterval))}reset(){this._isStopped=!1,this.tokens=this.bucketSize,this.tokensRemovedAt=[Date.now()],super.reset()}};var At=new Re("Flag"),rt=class extends He{constructor(r,i){super(r,"div","Flag");this.parent=r;this.hostname=i}async check(){let r=!0;try{let i=localStorage.getItem(this.hostname);if(i!=null){let c=JSON.parse(i),p=Date.now(),I=c.time,f=p-I,v=7*24*60*60*1e3;f<v&&(r=!1,this.onResponse(c))}}catch(i){throw At.error(`error while getting savedSTR for ${this.hostname}`,i),this.onError(),i}if(r){this.startLimiters();let i=await this.waitForAvailableEndpoint();await this.dnsRequest(i)}}startLimiters(){rt.googleLimiter.isStopped===!0&&rt.googleLimiter.start(),rt.cloudFlareLimiter.isStopped===!0&&rt.cloudFlareLimiter.start()}async waitForAvailableEndpoint(){let r=await Promise.race([rt.googleLimiter.awaitTokens(1).then(()=>rt.googleLimiter.tryRemoveTokens(1)).then(i=>{if(i)return`https://dns.google/resolve?name=${this.hostname}&type=A`}),rt.cloudFlareLimiter.awaitTokens(1).then(()=>rt.cloudFlareLimiter.tryRemoveTokens(1)).then(i=>{if(i)return`https://cloudflare-dns.com/dns-query?name=${this.hostname}&type=A`})]);return r??(At.info("we awaited tokens, but could not retrieve any.. restarting dnsRequest"),await this.waitForAvailableEndpoint())}async dnsRequest(r){try{let c=await(await fetch(r,{method:"GET",headers:{Accept:"application/dns-json"}})).json();await this.handleDnsQueryResponse(c)}catch(i){At.error("problem submitting DNS request",r,i),this.onError()}}async handleDnsQueryResponse(r){if(r.Answer==null)return At.error('Response does not contain the "Answer" property:',r),this.onError();let i=null;for(let c=0;c<r.Answer.length&&i==null;c++){let p=r.Answer[c];p.type===1&&(i=p.data)}if(i!=null)try{let c=await qn(kn,i);if(c?.country_code!=null){this.onResponse(c),c.time=Date.now();let p=JSON.stringify(c);localStorage.setItem(this.hostname,p)}else At.error("geoipResponse.country_code is null")}catch(c){At.error(`error while getting DNS A record for ${this.hostname}`,c),this.onError()}else At.error("IP is still null",r)}onError(){this.tag.empty()}onResponse(r){this.tag.style.setProperty("background-image",`url('https://ipfs.io/ipfs/QmaYjj5BHGAWfopTdE8ESzypbuthsZqTeqz9rEuh3EJZi6/${r.country_code.toLowerCase()}.svg')`),this.tag.title=r.country_name,this.tag.empty()}},Ct=rt;Ct.googleLimiter=new Ut({bucketSize:1,tokensPerInterval:1,interval:1e3*2,stopped:!0}),Ct.cloudFlareLimiter=new Ut({bucketSize:1,tokensPerInterval:1,interval:1e3*2,stopped:!0});var di=new Re("expectSubdomainRedirect");async function Kn(e){let{redirected:t,url:r}=await fetch(e.toString());di.debug("redirected: ",t),di.debug("responseUrl: ",r),t&&di.debug("definitely redirected");let{hostname:i}=new Qe(r);if(!i.startsWith(ht)){let c=`Expected ${e.toString()} to redirect to subdomain '${ht}' but instead received '${r}'`;throw di.debug(c),new Error(c)}}async function pi(e){return await new Promise((r,i)=>{let c=new Image,p=setTimeout(()=>{i(new Error(`Timeout when attempting to load img from '${c.src}`))},15e3),I=(f,v,D,w,B)=>{clearTimeout(p),B==null?i(new Error(`Unknown Error when attempting to load img from '${c.src}`)):i(B)};c.onerror=I,c.onload=()=>{clearTimeout(p),r()},c.src=e.toString()})}var da=new Re("Origin"),gi=class{constructor(t){this.parent=t;this.tag=new Ze("div","Origin")}async check(){let t=new Qe(this.parent.gateway),r=new Qe(`${t.protocol}//${ht}.ipfs.${t.hostname}/?now=${Date.now()}&filename=1x1.png#x-ipfs-companion-no-redirect`),i=new Qe(`${t.protocol}//${t.hostname}/ipfs/${ht}?now=${Date.now()}&filename=1x1.png#x-ipfs-companion-no-redirect`);await pi(r).then(async()=>await Kn(i)).then(()=>{this.tag.win(r.toString()),this.parent.tag.classList.add("origin")}).catch(c=>this.onerror(c))}onerror(t){throw da.error(t),this.tag.err(),t}};var Jn=Zt(ei());var{fetch:pa}=(0,Jn.default)(),Xi=new Re("Trustless"),mi=class extends ut{constructor(r){super(r,"div","Trustless");this.parent=r;this._className="Trustless";this._tagName="div"}async check(){let r=Date.now(),i=this.parent.gateway.replace(":hash",ft);this.parent.tag.classList.add("trustless");try{let c=await Promise.all(Di.map(async I=>{let f=`${i}?format=${I}&now=${r}#x-ipfs-companion-no-redirect`,v=await pa(f);return Boolean(v.headers.get("Content-Type")?.includes(`application/vnd.ipld.${I}`))})),p=Di.filter((I,f)=>!c[f]);if(p.length===0)this.tag.win();else{let I=`URL '${i} does not support the following Trustless response types: [${p.join(", ")}]`;throw Xi.debug(I),new Error(I)}}catch(c){throw Xi.error(c),this.onerror(),c}}checked(){Xi.warn("Not implemented yet")}onerror(){this.tag.err()}};var ga=new Re("Status"),yi=class extends He{constructor(r){super(r,"div","Status");this.parent=r;this._up=!1;this._down=!1}async check(){let r=new Qe(this.parent.gateway),i=new Qe(`${r.protocol}//${r.hostname}/ipfs/${ht}?now=${Date.now()}&filename=1x1.png#x-ipfs-companion-no-redirect`);await pi(i).catch(c=>{throw c!=null&&ga.error(this.parent.gateway,c),c})}get down(){return this._down}set down(r){!this.up&&!this.down&&(this._down=!0,this.tag.lose())}get up(){return this._up}set up(r){!this.up&&!this.down&&(this._up=!0,this.tag.global(),this.parent.tag.classList.add("online"))}onerror(){throw new Error("Not implemented")}};function Qi(e){let t=e.toString();return e?.hostname!=null&&(t=e.hostname.toString()),t.replace(`${ft}.ipfs.`,"").replace(`${ft}.`,"")}var Yn=Zt(ei());var{fetch:ma}=(0,Yn.default)(),Zi=new Re("Ipns"),wi=class extends ut{constructor(r){super(r,"div","Ipns");this.parent=r;this._className="Ipns";this._tagName="div"}async check(){let r=Date.now(),i=new URL(this.parent.gateway);i.pathname=En;let c=`${i.href}?now=${r}#x-ipfs-companion-no-redirect`;try{if((await ma(c)).status===200)this.tag.win();else throw Zi.debug(`${this.parent.gateway} does not support IPNS`),new Error(`URL '${c} is not reachable`)}catch(p){throw Zi.error(p),this.onerror(),p}}checked(){Zi.warn("Not implemented yet")}onerror(){this.tag.err()}};var Ge=new Re("GatewayNode"),_i=class extends He{constructor(r,i,c){super(r,"div","Node");this.parent=r;this.atLeastOneSuccess=!1;this.tag.empty(),this.tag.style.order=Date.now().toString(),this.status=new yi(this),this.tag.append(this.status.tag),this.cors=new ti(this),this.tag.append(this.cors.tag),this.ipns=new wi(this),this.tag.append(this.ipns.tag),this.origin=new gi(this),this.tag.append(this.origin.tag),this.trustless=new mi(this),this.tag.append(this.trustless.tag),this.link=document.createElement("div");let p=i.replace(":hash",ft);this.link.url=new Qe(p),this.link.textContent=Qi(this.link.url),this.link.className="Link",this.flag=new Ct(this,this.link.textContent),this.tag.append(this.flag.tag),this.tag.append(this.link),this.took=document.createElement("div"),this.took.className="Took",this.tag.append(this.took),this.gateway=i,this.index=c,this.checkingTime=0}async check(){this.checkingTime=Date.now(),this.flag.check().then(()=>Ge.debug(this.gateway,"Flag success"));let r=[this.status.check().then(()=>Ge.debug(this.gateway,"Status success")).then(this.onSuccessfulCheck.bind(this)),this.cors.check().then(()=>Ge.debug(this.gateway,"CORS success")).then(this.onSuccessfulCheck.bind(this)),this.ipns.check().then(()=>Ge.debug(this.gateway,"IPNS success")).then(this.onSuccessfulCheck.bind(this)),this.origin.check().then(()=>Ge.debug(this.gateway,"Origin success")).then(this.onSuccessfulCheck.bind(this)),this.trustless.check().then(()=>Ge.debug(this.gateway,"Trustless success")).then(this.onSuccessfulCheck.bind(this))];await Promise.allSettled(r).then(i=>i.map(c=>c.status)).then(i=>{i.includes("fulfilled")?(Ge.debug(`For gateway '${this.gateway}', at least one promise was successfully fulfilled`),Ge.debug(this.gateway,"this.status.up: ",this.status.up),Ge.debug(this.gateway,"this.status.down: ",this.status.down),this.status.up=!0,Ge.debug(this.gateway,"this.status.up: ",this.status.up),Ge.debug(this.gateway,"this.status.down: ",this.status.down)):(this.status.down=!0,Ge.debug(`For gateway '${this.gateway}', all promises were rejected`))})}onSuccessfulCheck(){if(!this.atLeastOneSuccess){Ge.info(`For gateway '${this.gateway}', at least one check was successful`),this.atLeastOneSuccess=!0,this.status.up=!0;let r=this.link.url;if(r!=null){let p=Qi(r);this.link.innerHTML=`<a title="${p}" href="${r.toString()}#x-ipfs-companion-no-redirect" target="_blank">${p}</a>`}let i=Date.now()-this.checkingTime;this.tag.style.order=i.toString();let c=(i/1e3).toFixed(2);this.took.textContent=`${c}s`}}onerror(){this.tag.err()}};var vi=class{constructor(t){this.parent=t;let r=document.getElementById("checker.results");if(r==null)throw new Error('Element with Id "checker.results" not found.');this.element=r}append(t){this.element.append(t.element)}};var bi=class extends He{constructor(r){super(r);this.parent=r;let i=document.getElementById("checker.stats");if(i==null)throw new Error('Could not find element with Id "checker.stats"');this.tag=Ze.fromElement(i),this.gateways=document.createElement("div"),this.gateways.textContent="0/0 tested",this.gateways.className="Gateways",this.tag.append(this.gateways),this.totals=document.createElement("div"),this.totals.textContent="0 online",this.totals.className="Totals",this.tag.append(this.totals)}update(){let r=0,i=0;for(let c of this.parent.nodes)c.status.up?r+=1:c.status.down&&(i+=1);this.gateways.textContent=`${r+i}/${this.parent.nodes.length} tested`,this.totals.textContent=`${r} online`}};var ya=new Re("Checker"),Ei=class{constructor(){this.nodes=[];let t=document.getElementById("checker");if(t==null)throw new Error('Element with Id "checker" not found.');this.element=t,this.stats=new bi(this),this.results=new vi(this),this.updateStats=this.updateStats.bind(this)}updateStats(){this.stats.update()}async checkGateways(t){let r=[];for(let i of t){let c=new _i(this.results,i,this.nodes.length);this.nodes.push(c),this.results.append(c.tag),setTimeout(()=>{r.push(c.check().catch(p=>ya.error(p)).finally(this.updateStats))},100*this.nodes.length)}}};var Xn=["https://ipfs.io/ipfs/:hash","https://dweb.link/ipfs/:hash","https://gateway.ipfs.io/ipfs/:hash","https://ninetailed.ninja/ipfs/:hash","https://via0.com/ipfs/:hash","https://ipfs.eternum.io/ipfs/:hash","https://hardbin.com/ipfs/:hash","https://cloudflare-ipfs.com/ipfs/:hash","https://astyanax.io/ipfs/:hash","https://cf-ipfs.com/ipfs/:hash","https://ipns.co/ipfs/:hash","https://gateway.originprotocol.com/ipfs/:hash","https://gateway.pinata.cloud/ipfs/:hash","https://ipfs.sloppyta.co/ipfs/:hash","https://ipfs.busy.org/ipfs/:hash","https://ipfs.greyh.at/ipfs/:hash","https://gateway.serph.network/ipfs/:hash","https://jorropo.net/ipfs/:hash","https://ipfs.fooock.com/ipfs/:hash","https://cdn.cwinfo.net/ipfs/:hash","https://aragon.ventures/ipfs/:hash","https://permaweb.io/ipfs/:hash","https://ipfs.best-practice.se/ipfs/:hash","https://storjipfs-gateway.com/ipfs/:hash","https://ipfs.runfission.com/ipfs/:hash","https://ipfs.trusti.id/ipfs/:hash","https://ipfs.overpi.com/ipfs/:hash","https://ipfs.ink/ipfs/:hash","https://ipfsgateway.makersplace.com/ipfs/:hash","https://ipfs.funnychain.co/ipfs/:hash","https://ipfs.telos.miami/ipfs/:hash","https://ipfs.mttk.net/ipfs/:hash","https://ipfs.fleek.co/ipfs/:hash","https://ipfs.jbb.one/ipfs/:hash","https://ipfs.yt/ipfs/:hash","https://hashnews.k1ic.com/ipfs/:hash","https://ipfs.drink.cafe/ipfs/:hash","https://ipfs.kavin.rocks/ipfs/:hash","https://ipfs.denarius.io/ipfs/:hash","https://crustwebsites.net/ipfs/:hash","https://ipfs0.sjc.cloudsigma.com/ipfs/:hash","http://ipfs.genenetwork.org/ipfs/:hash","https://ipfs.eth.aragon.network/ipfs/:hash","https://ipfs.smartholdem.io/ipfs/:hash","https://ipfs.xoqq.ch/ipfs/:hash","http://natoboram.mynetgear.com:8080/ipfs/:hash","https://video.oneloveipfs.com/ipfs/:hash","http://ipfs.anonymize.com/ipfs/:hash","https://ipfs.scalaproject.io/ipfs/:hash","https://search.ipfsgate.com/ipfs/:hash","https://ipfs.decoo.io/ipfs/:hash","https://alexdav.id/ipfs/:hash","https://ipfs.uploads.nu/ipfs/:hash","https://hub.textile.io/ipfs/:hash","https://ipfs1.pixura.io/ipfs/:hash","https://ravencoinipfs-gateway.com/ipfs/:hash","https://konubinix.eu/ipfs/:hash","https://ipfs.tubby.cloud/ipfs/:hash","https://ipfs.lain.la/ipfs/:hash","https://ipfs.kaleido.art/ipfs/:hash","https://ipfs.slang.cx/ipfs/:hash","https://ipfs.arching-kaos.com/ipfs/:hash","https://storry.tv/ipfs/:hash","https://ipfs.1-2.dev/ipfs/:hash","https://dweb.eu.org/ipfs/:hash","https://permaweb.eu.org/ipfs/:hash","https://ipfs.namebase.io/ipfs/:hash","https://ipfs.tribecap.co/ipfs/:hash","https://ipfs.kinematiks.com/ipfs/:hash","https://c4rex.co/ipfs/:hash","https://nftstorage.link/ipfs/:hash","https://gravity.jup.io/ipfs/:hash","http://fzdqwfb5ml56oadins5jpuhe6ki6bk33umri35p5kt2tue4fpws5efid.onion/ipfs/:hash","https://tth-ipfs.com/ipfs/:hash","https://ipfs.chisdealhd.co.uk/ipfs/:hash","https://ipfs.alloyxuast.tk/ipfs/:hash","https://ipfs.litnet.work/ipfs/:hash","https://4everland.io/ipfs/:hash","https://ipfs-gateway.cloud/ipfs/:hash","https://w3s.link/ipfs/:hash","https://cthd.icu/ipfs/:hash","https://ipfs.joaoleitao.org/ipfs/:hash","https://ipfs.tayfundogdas.me/ipfs/:hash","https://ipfs.jpu.jp/ipfs/:hash"];var Ke=Zt(Zn()),er=localStorage.getItem("metrics_consent"),tr=document.querySelector(".js-metrics-notification"),tn=document.querySelector(".js-metrics-notification-decline-warning"),nn=document.querySelector(".js-metrics-notification-accept"),ir=document.querySelector(".js-metrics-notification-decline"),nr=document.querySelector(".js-metrics-notification-warning-close"),qt=document.querySelector(".js-cookie-banner-toggle");function rn(e){sr(),Ke.default.add_consent(e),Array.isArray(e)?localStorage.setItem("metrics_consent",JSON.stringify(e)):localStorage.setItem("metrics_consent",JSON.stringify([e]))}function on(){nn?.removeEventListener("click",on),rn(["all"])}function rr(){rn(["necessary"]),sr(),_a()}function _a(){tn?.classList.remove("hidden"),qt?.setAttribute("disabled","")}function or(){nr?.removeEventListener("click",or),tn?.classList.add("hidden"),qt?.removeAttribute("disabled")}function sr(){nn?.removeEventListener("click",on),ir?.removeEventListener("click",rr),tr?.classList.add("hidden"),qt?.removeAttribute("disabled")}function en(){nn?.addEventListener("click",on),ir?.addEventListener("click",rr),nr?.addEventListener("click",or),tr?.classList.remove("hidden"),qt?.setAttribute("disabled",""),tn?.classList.add("hidden")}function va(){en()}function ar(){qt?.addEventListener("click",va),Ke.default.init({app_key:"3c2c0819434074fc4d339ddd8e112a1e741ecb72",url:"https://countly.ipfs.io",require_consent:!0});let e=["sessions","views"],t=["attribution","users","location"],r=["events","crashes","apm"],i=["scrolls","clicks","forms","star-rating","feedback"];if(Ke.default.group_features({all:[...e,...t,...r,...i],necessary:e,marketing:t,tracking:i,performance:r}),Ke.default.track_clicks(),Ke.default.track_errors(),Ke.default.track_forms(),Ke.default.track_links(),Ke.default.track_pageview(),Ke.default.track_scrolls(),Ke.default.track_sessions(),Ke.default.track_view(),er!=null)try{rn(JSON.parse(er))}catch{en()}else en()}try{self["workbox:window:6.5.3"]&&_()}catch{}function fr(e,t){return new Promise(function(r){var i=new MessageChannel;i.port1.onmessage=function(c){r(c.data)},e.postMessage(t,[i.port2])})}function cr(e,t){for(var r=0;r<t.length;r++){var i=t[r];i.enumerable=i.enumerable||!1,i.configurable=!0,"value"in i&&(i.writable=!0),Object.defineProperty(e,i.key,i)}}function lr(e,t){(t==null||t>e.length)&&(t=e.length);for(var r=0,i=new Array(t);r<t;r++)i[r]=e[r];return i}function ba(e,t){var r;if(typeof Symbol>"u"||e[Symbol.iterator]==null){if(Array.isArray(e)||(r=function(c,p){if(c){if(typeof c=="string")return lr(c,p);var I=Object.prototype.toString.call(c).slice(8,-1);return I==="Object"&&c.constructor&&(I=c.constructor.name),I==="Map"||I==="Set"?Array.from(c):I==="Arguments"||/^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(I)?lr(c,p):void 0}}(e))||t&&e&&typeof e.length=="number"){r&&(e=r);var i=0;return function(){return i>=e.length?{done:!0}:{done:!1,value:e[i++]}}}throw new TypeError(`Invalid attempt to iterate non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}return(r=e[Symbol.iterator]()).next.bind(r)}try{self["workbox:core:6.5.3"]&&_()}catch{}var sn=function(){var e=this;this.promise=new Promise(function(t,r){e.resolve=t,e.reject=r})};function an(e,t){var r=location.href;return new URL(e,r).href===new URL(t,r).href}var Kt=function(e,t){this.type=e,Object.assign(this,t)};function Si(e,t,r){return r?t?t(e):e:(e&&e.then||(e=Promise.resolve(e)),t?e.then(t):e)}function Ea(){}var ka={type:"SKIP_WAITING"};function ur(e,t){if(!t)return e&&e.then?e.then(Ea):Promise.resolve()}var hr=function(e){var t,r;function i(v,D){var w,B;return D===void 0&&(D={}),(w=e.call(this)||this).nn={},w.tn=0,w.rn=new sn,w.en=new sn,w.on=new sn,w.un=0,w.an=new Set,w.cn=function(){var T=w.fn,M=T.installing;w.tn>0||!an(M.scriptURL,w.sn.toString())||performance.now()>w.un+6e4?(w.vn=M,T.removeEventListener("updatefound",w.cn)):(w.hn=M,w.an.add(M),w.rn.resolve(M)),++w.tn,M.addEventListener("statechange",w.ln)},w.ln=function(T){var M=w.fn,S=T.target,b=S.state,F=S===w.vn,K={sw:S,isExternal:F,originalEvent:T};!F&&w.mn&&(K.isUpdate=!0),w.dispatchEvent(new Kt(b,K)),b==="installed"?w.wn=self.setTimeout(function(){b==="installed"&&M.waiting===S&&w.dispatchEvent(new Kt("waiting",K))},200):b==="activating"&&(clearTimeout(w.wn),F||w.en.resolve(S))},w.dn=function(T){var M=w.hn,S=M!==navigator.serviceWorker.controller;w.dispatchEvent(new Kt("controlling",{isExternal:S,originalEvent:T,sw:M,isUpdate:w.mn})),S||w.on.resolve(M)},w.gn=(B=function(T){var M=T.data,S=T.ports,b=T.source;return Si(w.getSW(),function(){w.an.has(b)&&w.dispatchEvent(new Kt("message",{data:M,originalEvent:T,ports:S,sw:b}))})},function(){for(var T=[],M=0;M<arguments.length;M++)T[M]=arguments[M];try{return Promise.resolve(B.apply(this,T))}catch(S){return Promise.reject(S)}}),w.sn=v,w.nn=D,navigator.serviceWorker.addEventListener("message",w.gn),w}r=e,(t=i).prototype=Object.create(r.prototype),t.prototype.constructor=t,t.__proto__=r;var c,p,I,f=i.prototype;return f.register=function(v){var D=(v===void 0?{}:v).immediate,w=D!==void 0&&D;try{var B=this;return function(T,M){var S=T();return S&&S.then?S.then(M):M(S)}(function(){if(!w&&document.readyState!=="complete")return ur(new Promise(function(T){return window.addEventListener("load",T)}))},function(){return B.mn=Boolean(navigator.serviceWorker.controller),B.yn=B.pn(),Si(B.bn(),function(T){B.fn=T,B.yn&&(B.hn=B.yn,B.en.resolve(B.yn),B.on.resolve(B.yn),B.yn.addEventListener("statechange",B.ln,{once:!0}));var M=B.fn.waiting;return M&&an(M.scriptURL,B.sn.toString())&&(B.hn=M,Promise.resolve().then(function(){B.dispatchEvent(new Kt("waiting",{sw:M,wasWaitingBeforeRegister:!0}))}).then(function(){})),B.hn&&(B.rn.resolve(B.hn),B.an.add(B.hn)),B.fn.addEventListener("updatefound",B.cn),navigator.serviceWorker.addEventListener("controllerchange",B.dn),B.fn})})}catch(T){return Promise.reject(T)}},f.update=function(){try{return this.fn?ur(this.fn.update()):void 0}catch(v){return Promise.reject(v)}},f.getSW=function(){return this.hn!==void 0?Promise.resolve(this.hn):this.rn.promise},f.messageSW=function(v){try{return Si(this.getSW(),function(D){return fr(D,v)})}catch(D){return Promise.reject(D)}},f.messageSkipWaiting=function(){this.fn&&this.fn.waiting&&fr(this.fn.waiting,ka)},f.pn=function(){var v=navigator.serviceWorker.controller;return v&&an(v.scriptURL,this.sn.toString())?v:void 0},f.bn=function(){try{var v=this;return function(D,w){try{var B=D()}catch(T){return w(T)}return B&&B.then?B.then(void 0,w):B}(function(){return Si(navigator.serviceWorker.register(v.sn,v.nn),function(D){return v.un=performance.now(),D})},function(D){throw D})}catch(D){return Promise.reject(D)}},c=i,(p=[{key:"active",get:function(){return this.en.promise}},{key:"controlling",get:function(){return this.on.promise}}])&&cr(c.prototype,p),I&&cr(c,I),i}(function(){function e(){this.Pn=new Map}var t=e.prototype;return t.addEventListener=function(r,i){this.Sn(r).add(i)},t.removeEventListener=function(r,i){this.Sn(r).delete(i)},t.dispatchEvent=function(r){r.target=this;for(var i,c=ba(this.Sn(r.type));!(i=c()).done;)(0,i.value)(r)},t.Sn=function(r){return this.Pn.has(r)||this.Pn.set(r,new Set),this.Pn.get(r)},e}());var Sa=new hr("/sw.js");Sa.register();ar();var dr=new Re("App index");window.checker=new Ei;window.checker.checkGateways(Xn).catch(e=>{dr.error("Unexpected error"),dr.error(e)});})();
/*!
