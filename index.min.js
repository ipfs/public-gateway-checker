...
`,ia=42;function na(e){if(e[0]!==0)throw new Error("Invalid CID for CBOR tag 42; expected leading 0x00");return Ne.decode(e.subarray(1))}var qn={allowIndefinite:!1,coerceUndefinedToNull:!0,allowNaN:!1,allowInfinity:!1,allowBigInt:!0,strict:!0,useMaps:!1,tags:[]};qn.tags[ia]=na;var ra=e=>ms(e,qn),oa=Cn($r(),1),sa="Earth";function aa(e){return{country_name:e[0],country_code:e[1],region_code:e[2],city:e[3],postal_code:e[4],latitude:e[5],longitude:e[6],planet:sa}}var fa=Ne.parse("bafyreihnpl7ami7esahkfdnemm6idx4r2n6u3apmtcrxlqwuapgjsciihy"),ca=["https://ipfs.io","https://dweb.link"];async function la(e,t){if(typeof e=="function")return e(t);if(typeof e?.block?.get=="function")return e.block.get(t);let r=Array.isArray(e)?e:[e];for(let i of r){let c=new URL(i);c.pathname=`/ipfs/${t.toString()}`,c.search="?format=raw";try{let p=await(0,oa.default)(c,{headers:{Accept:"application/vnd.ipld.raw"},cache:"force-cache"});if(!p.ok)throw p;return new Uint8Array(await p.arrayBuffer())}catch(p){throw new Error(`unable to fetch raw block for CID ${t}`,{cause:p})}}}async function Kn(e,t,r=1){try{let i=await la(e,t);return{obj:await ra(i),block:i}}catch(i){if(r<3)return await Kn(e,t,r+1);throw i}}async function ua(e,t,r){let i,c;try{({obj:i,block:c}=await Kn(e,t))}catch(I){throw(process?.env?.DEBUG||process?.env?.TEST)&&console.error(c?`[ipfs-geoip] failed to parse DAG-CBOR behind CID '${t}'`:`[ipfs-geoip] failed to get raw block for CID '${t}'`,I),I}let p=0;if("data"in i){if("data"in i){for(;i.data[p]&&i.data[p].min<=r;)p++;let I=i.data[p-1];if(!I)throw new Error("Failed to lookup leaf node");if(!I.data)throw new Error("Unmapped range");return aa(I.data)}}else{for(;i.mins[p]&&i.mins[p]<=r;)p++;let I=i.links[p-1];if(!I)throw new Error("Failed to lookup node");let f=ha(I);if(!f)throw new Error("Failed to lookup node");return Jn(e,f,r)}}var Jn=Qr(ua,{cachePromiseRejection:!1,cacheKey:e=>{let[,t,r]=e;return`${t}.${r}`}});function Yn(e=ca,t){return Jn(e,fa,Zr.default.toLong(t))}function ha(e){return e?Ne.asCID(e):null}var At=new Re("Flag"),rt=class extends We{constructor(r,i){super(r,"div","Flag");this.parent=r;this.hostname=i}async check(){let r=!0;try{let i=localStorage.getItem(this.hostname);if(i!=null){let c=JSON.parse(i),p=Date.now(),I=c.time,f=p-I,v=7*24*60*60*1e3;f<v&&(r=!1,this.onResponse(c))}}catch(i){throw At.error(`error while getting savedSTR for ${this.hostname}`,i),this.onError(),i}if(r){this.startLimiters();let i=await this.waitForAvailableEndpoint();await this.dnsRequest(i)}}startLimiters(){rt.googleLimiter.isStopped===!0&&rt.googleLimiter.start(),rt.cloudFlareLimiter.isStopped===!0&&rt.cloudFlareLimiter.start()}async waitForAvailableEndpoint(){let r=await Promise.race([rt.googleLimiter.awaitTokens(1).then(()=>rt.googleLimiter.tryRemoveTokens(1)).then(i=>{if(i)return`https://dns.google/resolve?name=${this.hostname}&type=A`}),rt.cloudFlareLimiter.awaitTokens(1).then(()=>rt.cloudFlareLimiter.tryRemoveTokens(1)).then(i=>{if(i)return`https://cloudflare-dns.com/dns-query?name=${this.hostname}&type=A`})]);return r??(At.info("we awaited tokens, but could not retrieve any.. restarting dnsRequest"),this.waitForAvailableEndpoint())}async dnsRequest(r){try{let c=await(await fetch(r,{method:"GET",headers:{Accept:"application/dns-json"}})).json();await this.handleDnsQueryResponse(c)}catch(i){At.error("problem submitting DNS request",r,i),this.onError()}}async handleDnsQueryResponse(r){if(r.Answer==null){At.error('Response does not contain the "Answer" property:',r),this.onError();return}let i=null;for(let c=0;c<r.Answer.length&&i==null;c++){let p=r.Answer[c];p.type===1&&(i=p.data)}if(i!=null)try{let c=await Yn(In,i);if(c?.country_code!=null){this.onResponse(c),c.time=Date.now();let p=JSON.stringify(c);localStorage.setItem(this.hostname,p)}else At.error("geoipResponse.country_code is null")}catch(c){At.error(`error while getting DNS A record for ${this.hostname}`,c),this.onError()}else At.error("IP is still null",r)}onError(){this.tag.empty()}onResponse(r){this.tag.style.setProperty("background-image",`url('https://ipfs.io/ipfs/QmaYjj5BHGAWfopTdE8ESzypbuthsZqTeqz9rEuh3EJZi6/${r.country_code.toLowerCase()}.svg')`),this.tag.title=r.country_name,this.tag.empty()}},Ct=rt;Ct.googleLimiter=new Tt({bucketSize:1,tokensPerInterval:1,interval:1e3*2,stopped:!0}),Ct.cloudFlareLimiter=new Tt({bucketSize:1,tokensPerInterval:1,interval:1e3*2,stopped:!0});var Xn=Qt(ei());var{fetch:da}=(0,Xn.default)(),Zi=new Re("Ipns"),di=class extends ut{constructor(r){super(r,"div","Ipns");this.parent=r;this._className="Ipns";this._tagName="div"}async check(){let r=Date.now(),i=new URL(this.parent.gateway);i.pathname=Rn;let c=`${i.href}?now=${r}#x-ipfs-companion-no-redirect`;try{if((await da(c)).status===200)this.tag.win();else throw Zi.debug(`${this.parent.gateway} does not support IPNS`),new Error(`URL '${c} is not reachable`)}catch(p){throw Zi.error(p),this.onerror(),p}}checked(){Zi.warn("Not implemented yet")}onerror(){this.tag.err()}};async function pi(e){await new Promise((r,i)=>{let c=new Image,p=setTimeout(()=>{i(new Error(`Timeout when attempting to load img from '${c.src}`))},15e3),I=(f,v,D,w,B)=>{clearTimeout(p),B==null?i(new Error(`Unknown Error when attempting to load img from '${c.src}`)):i(B)};c.onerror=I,c.onload=()=>{clearTimeout(p),r()},c.src=e.toString()})}var gi=new Re("expectSubdomainRedirect");async function Qn(e){let{redirected:t,url:r}=await fetch(e.toString());gi.debug("redirected: ",t),gi.debug("responseUrl: ",r),t&&gi.debug("definitely redirected");let{hostname:i}=new Qe(r);if(!i.startsWith(ht)){let c=`Expected ${e.toString()} to redirect to subdomain '${ht}' but instead received '${r}'`;throw gi.debug(c),new Error(c)}}var pa=new Re("Origin"),mi=class{constructor(t){this.parent=t;this.tag=new Ze("div","Origin")}async check(){let t=new Qe(this.parent.gateway),r=new Qe(`${t.protocol}//${ht}.ipfs.${t.hostname}/?now=${Date.now()}&filename=1x1.png#x-ipfs-companion-no-redirect`),i=new Qe(`${t.protocol}//${t.hostname}/ipfs/${ht}?now=${Date.now()}&filename=1x1.png#x-ipfs-companion-no-redirect`);await pi(r).then(async()=>Qn(i)).then(()=>{this.tag.win(r.toString()),this.parent.tag.classList.add("origin")}).catch(c=>{this.onerror(c)})}onerror(t){throw pa.error(t),this.tag.err(),t}};var ga=new Re("Status"),yi=class extends We{constructor(r){super(r,"div","Status");this.parent=r;this._up=!1;this._down=!1}async check(){let r=new Qe(this.parent.gateway),i=new Qe(`${r.protocol}//${r.hostname}/ipfs/${ht}?now=${Date.now()}&filename=1x1.png#x-ipfs-companion-no-redirect`);await pi(i).catch(c=>{throw c!=null&&ga.error(this.parent.gateway,c),c})}get down(){return this._down}set down(r){!this.up&&!this.down&&(this._down=!0,this.tag.lose())}get up(){return this._up}set up(r){!this.up&&!this.down&&(this._up=!0,this.tag.global(),this.parent.tag.classList.add("online"))}onerror(){throw new Error("Not implemented")}};var Zn=Qt(ei());var{fetch:ma}=(0,Zn.default)(),en=new Re("Trustless"),wi=class extends ut{constructor(r){super(r,"div","Trustless");this.parent=r;this._className="Trustless";this._tagName="div"}async check(){let r=Date.now(),i=this.parent.gateway.replace(":hash",ft);this.parent.tag.classList.add("trustless");try{let c=await Promise.all(Ui.map(async I=>{let f=`${i}?format=${I}&now=${r}#x-ipfs-companion-no-redirect`,v=await ma(f);return Boolean(v.headers.get("Content-Type")?.includes(`application/vnd.ipld.${I}`))})),p=Ui.filter((I,f)=>!c[f]);if(p.length===0)this.tag.win();else{let I=`URL '${i} does not support the following Trustless response types: [${p.join(", ")}]`;throw en.debug(I),new Error(I)}}catch(c){throw en.error(c),this.onerror(),c}}checked(){en.warn("Not implemented yet")}onerror(){this.tag.err()}};function tn(e){let t=e.toString();return e?.hostname!=null&&(t=e.hostname.toString()),t.replace(`${ft}.ipfs.`,"").replace(`${ft}.`,"")}var Ge=new Re("GatewayNode"),_i=class extends We{constructor(r,i,c){super(r,"div","Node");this.parent=r;this.atLeastOneSuccess=!1;this.tag.empty(),this.tag.style.order=Date.now().toString(),this.status=new yi(this),this.tag.append(this.status.tag),this.cors=new ti(this),this.tag.append(this.cors.tag),this.ipns=new di(this),this.tag.append(this.ipns.tag),this.origin=new mi(this),this.tag.append(this.origin.tag),this.trustless=new wi(this),this.tag.append(this.trustless.tag),this.link=document.createElement("div");let p=i.replace(":hash",ft);this.link.url=new Qe(p),this.link.textContent=tn(this.link.url),this.link.className="Link",this.flag=new Ct(this,this.link.textContent),this.tag.append(this.flag.tag),this.tag.append(this.link),this.took=document.createElement("div"),this.took.className="Took",this.tag.append(this.took),this.gateway=i,this.index=c,this.checkingTime=0}async check(){this.checkingTime=Date.now(),this.flag.check().then(()=>{Ge.debug(this.gateway,"Flag success")});let r=[this.status.check().then(()=>{Ge.debug(this.gateway,"Status success")}).then(this.onSuccessfulCheck.bind(this)),this.cors.check().then(()=>{Ge.debug(this.gateway,"CORS success")}).then(this.onSuccessfulCheck.bind(this)),this.ipns.check().then(()=>{Ge.debug(this.gateway,"IPNS success")}).then(this.onSuccessfulCheck.bind(this)),this.origin.check().then(()=>{Ge.debug(this.gateway,"Origin success")}).then(this.onSuccessfulCheck.bind(this)),this.trustless.check().then(()=>{Ge.debug(this.gateway,"Trustless success")}).then(this.onSuccessfulCheck.bind(this))];await Promise.allSettled(r).then(i=>i.map(c=>c.status)).then(i=>{i.includes("fulfilled")?(Ge.debug(`For gateway '${this.gateway}', at least one promise was successfully fulfilled`),Ge.debug(this.gateway,"this.status.up: ",this.status.up),Ge.debug(this.gateway,"this.status.down: ",this.status.down),this.status.up=!0,Ge.debug(this.gateway,"this.status.up: ",this.status.up),Ge.debug(this.gateway,"this.status.down: ",this.status.down)):(this.status.down=!0,Ge.debug(`For gateway '${this.gateway}', all promises were rejected`))})}onSuccessfulCheck(){if(!this.atLeastOneSuccess){Ge.info(`For gateway '${this.gateway}', at least one check was successful`),this.atLeastOneSuccess=!0,this.status.up=!0;let r=this.link.url;if(r!=null){let p=tn(r);this.link.innerHTML=`<a title="${p}" href="${r.toString()}#x-ipfs-companion-no-redirect" target="_blank">${p}</a>`}let i=Date.now()-this.checkingTime;this.tag.style.order=i.toString();let c=(i/1e3).toFixed(2);this.took.textContent=`${c}s`}}onerror(){this.tag.err()}};var vi=class{constructor(t){this.parent=t;let r=document.getElementById("checker.results");if(r==null)throw new Error('Element with Id "checker.results" not found.');this.element=r}append(t){this.element.append(t.element)}};var bi=class extends We{constructor(r){super(r);this.parent=r;let i=document.getElementById("checker.stats");if(i==null)throw new Error('Could not find element with Id "checker.stats"');this.tag=Ze.fromElement(i),this.gateways=document.createElement("div"),this.gateways.textContent="0/0 tested",this.gateways.className="Gateways",this.tag.append(this.gateways),this.totals=document.createElement("div"),this.totals.textContent="0 online",this.totals.className="Totals",this.tag.append(this.totals)}update(){let r=0,i=0;for(let c of this.parent.nodes)c.status.up?r+=1:c.status.down&&(i+=1);this.gateways.textContent=`${r+i}/${this.parent.nodes.length} tested`,this.totals.textContent=`${r} online`}};var ya=new Re("Checker"),Ei=class{constructor(){this.nodes=[];let t=document.getElementById("checker");if(t==null)throw new Error('Element with Id "checker" not found.');this.element=t,this.stats=new bi(this),this.results=new vi(this),this.updateStats=this.updateStats.bind(this)}updateStats(){this.stats.update()}async checkGateways(t){let r=[];for(let i of t){let c=new _i(this.results,i,this.nodes.length);this.nodes.push(c),this.results.append(c.tag),setTimeout(()=>{r.push(c.check().catch(p=>{ya.error(p)}).finally(this.updateStats))},100*this.nodes.length)}}};var er=["https://ipfs.io/ipfs/:hash","https://dweb.link/ipfs/:hash","https://gateway.ipfs.io/ipfs/:hash","https://ninetailed.ninja/ipfs/:hash","https://via0.com/ipfs/:hash","https://ipfs.eternum.io/ipfs/:hash","https://hardbin.com/ipfs/:hash","https://cloudflare-ipfs.com/ipfs/:hash","https://astyanax.io/ipfs/:hash","https://cf-ipfs.com/ipfs/:hash","https://ipns.co/ipfs/:hash","https://gateway.originprotocol.com/ipfs/:hash","https://gateway.pinata.cloud/ipfs/:hash","https://ipfs.sloppyta.co/ipfs/:hash","https://ipfs.busy.org/ipfs/:hash","https://ipfs.greyh.at/ipfs/:hash","https://gateway.serph.network/ipfs/:hash","https://gw3.io/ipfs/:hash","https://jorropo.net/ipfs/:hash","https://ipfs.fooock.com/ipfs/:hash","https://cdn.cwinfo.net/ipfs/:hash","https://aragon.ventures/ipfs/:hash","https://permaweb.io/ipfs/:hash","https://ipfs.best-practice.se/ipfs/:hash","https://storjipfs-gateway.com/ipfs/:hash","https://ipfs.runfission.com/ipfs/:hash","https://ipfs.trusti.id/ipfs/:hash","https://ipfs.overpi.com/ipfs/:hash","https://ipfs.ink/ipfs/:hash","https://ipfsgateway.makersplace.com/ipfs/:hash","https://ipfs.funnychain.co/ipfs/:hash","https://ipfs.telos.miami/ipfs/:hash","https://ipfs.mttk.net/ipfs/:hash","https://ipfs.fleek.co/ipfs/:hash","https://ipfs.jbb.one/ipfs/:hash","https://ipfs.yt/ipfs/:hash","https://hashnews.k1ic.com/ipfs/:hash","https://ipfs.drink.cafe/ipfs/:hash","https://ipfs.kavin.rocks/ipfs/:hash","https://ipfs.denarius.io/ipfs/:hash","https://crustwebsites.net/ipfs/:hash","https://ipfs0.sjc.cloudsigma.com/ipfs/:hash","http://ipfs.genenetwork.org/ipfs/:hash","https://ipfs.eth.aragon.network/ipfs/:hash","https://ipfs.smartholdem.io/ipfs/:hash","https://ipfs.xoqq.ch/ipfs/:hash","http://natoboram.mynetgear.com:8080/ipfs/:hash","https://video.oneloveipfs.com/ipfs/:hash","http://ipfs.anonymize.com/ipfs/:hash","https://ipfs.scalaproject.io/ipfs/:hash","https://search.ipfsgate.com/ipfs/:hash","https://ipfs.decoo.io/ipfs/:hash","https://alexdav.id/ipfs/:hash","https://ipfs.uploads.nu/ipfs/:hash","https://hub.textile.io/ipfs/:hash","https://ipfs1.pixura.io/ipfs/:hash","https://ravencoinipfs-gateway.com/ipfs/:hash","https://konubinix.eu/ipfs/:hash","https://ipfs.tubby.cloud/ipfs/:hash","https://ipfs.lain.la/ipfs/:hash","https://ipfs.kaleido.art/ipfs/:hash","https://ipfs.slang.cx/ipfs/:hash","https://ipfs.arching-kaos.com/ipfs/:hash","https://storry.tv/ipfs/:hash","https://ipfs.1-2.dev/ipfs/:hash","https://dweb.eu.org/ipfs/:hash","https://permaweb.eu.org/ipfs/:hash","https://ipfs.namebase.io/ipfs/:hash","https://ipfs.tribecap.co/ipfs/:hash","https://ipfs.kinematiks.com/ipfs/:hash","https://nftstorage.link/ipfs/:hash","https://gravity.jup.io/ipfs/:hash","http://fzdqwfb5ml56oadins5jpuhe6ki6bk33umri35p5kt2tue4fpws5efid.onion/ipfs/:hash","https://tth-ipfs.com/ipfs/:hash","https://ipfs.chisdealhd.co.uk/ipfs/:hash","https://ipfs.alloyxuast.tk/ipfs/:hash","https://4everland.io/ipfs/:hash","https://ipfs-gateway.cloud/ipfs/:hash","https://w3s.link/ipfs/:hash","https://cthd.icu/ipfs/:hash","https://ipfs.tayfundogdas.me/ipfs/:hash","https://ipfs.jpu.jp/ipfs/:hash","https://ipfs.soul-network.com/ipfs/:hash","https://ipfs.twdragon.net/ipfs/:hash","https://getipfs.com/ipfs/:hash"];var ze=Qt(ir()),nr=localStorage.getItem("metrics_consent"),rn=document.querySelector(".js-metrics-notification-modal"),on=document.querySelector(".js-metrics-agreement"),ar=document.querySelector(".js-metrics-preferences"),wa=document.querySelector(".js-modal-close"),rr=document.querySelector(".js-metrics-notification-confirm"),or=document.querySelector(".js-metrics-notification-preferences-save"),sr=document.querySelector(".js-metrics-notification-manage"),nn=document.querySelector(".js-necessary-toggle"),_a=document.querySelector(".js-metrics-modal-toggle");function Si(e){ze.default.add_consent(e),Array.isArray(e)?localStorage.setItem("metrics_consent",JSON.stringify(e)):localStorage.setItem("metrics_consent",JSON.stringify([e]))}function va(){rn?.classList.add("hidden"),Si(["minimal"])}function ba(){nn.checked?Si(["minimal"]):(ze.default.remove_consent(["minimal"]),localStorage.setItem("metrics_consent",JSON.stringify([])))}function Ea(){rn?.classList.remove("hidden"),rr?.classList.remove("hidden"),sr?.classList.remove("hidden"),on?.classList.remove("hidden"),wa?.addEventListener("click",fr),rr?.addEventListener("click",va),sr?.addEventListener("click",ka)}function fr(){rn?.classList.add("hidden"),ar?.classList.add("hidden"),on?.classList.remove("hidden")}function ka(){let e=localStorage.getItem("metrics_consent");e!=null&&(nn.checked=JSON.parse(e)[0]==="minimal"),on?.classList.add("hidden"),or?.classList.remove("hidden"),ar?.classList.remove("hidden"),nn.addEventListener("click",ba),or?.addEventListener("click",fr)}function Sa(){Ea()}function cr(){_a?.addEventListener("click",Sa),ze.default.init({app_key:"3c2c0819434074fc4d339ddd8e112a1e741ecb72",url:"https://countly.ipfs.io",require_consent:!0});let e=["sessions","views","events"],t=["crashes","apm"],r=["scrolls","clicks","forms"],i=["star-rating","feedback"],c=["location"];ze.default.group_features({all:[...e,...t,...r,...i,...c],minimal:e,performance:t,ux:r,feedback:i,location:c}),ze.default.track_clicks(),ze.default.track_errors(),ze.default.track_forms(),ze.default.track_links(),ze.default.track_pageview(),ze.default.track_scrolls(),ze.default.track_sessions(),ze.default.track_view(),nr!=null?Si(JSON.parse(nr)):Si(["minimal"])}var Ra=new kn("/sw.js");Ra.register();cr();var lr=new Re("App index");window.checker=new Ei;window.checker.checkGateways(er).catch(e=>{lr.error("Unexpected error"),lr.error(e)});})();
